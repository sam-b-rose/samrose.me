...
Adding brotli compression to our Storybook configuration is made easier thanks to the to the [brotli-webpack-plugin](https://www.npmjs.com/package/brotli-webpack-plugin). Install the plugin package as a dev dependency.

```bash
npm install --save-dev brotli-webpack-plugin
```

Then, add the plugin to your `main.js` webpack config for Storybook.

```js
const BrotliPlugin = require('brotli-webpack-plugin');

module.exports = {
  // Optionally add this feature to enable code-splitting per story (^6.4)
  features: {
    storyStoreV7: true,
  },
...
When running the build script for Storybook, the output will include `*.(js|css|html|svg)` files as usual. With the plugin installed, it should now also generate `*.br.(js|css|html|svg)` assets which will be preferred and served by brotli compatible static servers. One way we can test the Storybook output is by using the `[http-server](https://github.com/http-party/http-server)` with the `-b` or `--brotli` flag enabled. The network tab in your browser’s inspector (dev tools) can be used to verify that the brotli encoded version is being properly delivered.
